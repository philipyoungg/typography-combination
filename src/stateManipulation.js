...
const cartesianObject = lift(map)(
  compose(zipObj, keys),
  compose(cartesianProduct, values)
);
...
const insideCombination = lift(zipObj)(
  keys,
  compose(map(cartesianObject), propsFromSchema)
);

// allCombination :: [{k: v}] -> [{k: v}]
export const allCombination = memoize(
  compose(cartesianObject, insideCombination)
);

// countTotalPermutation :: [{k: v}] -> Number
export const countTotalPermutation = compose(
  product,
  map(length),
  values,
  prop("properties")
...
export const convertToSchema = compose(
  wrapWithProperties,
  map(map(of)),
  filter(is(Object))
);
