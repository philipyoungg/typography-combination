...
const view = {
  render: {
    combination: (store, handlers) => {
      const { state } = store;
      $("#app").empty().append(
        $.map(state.combination, (item, index) =>
          $("<div>")
            .addClass("typography")
              ...
              $("<div>")
                ...
                .append(
                  $("<p>")
                    ...
                    .attr("combination-index", index)
                    .css({
                      textDecoration: state.totalPermutation === 1
                        ? "line-through"
                        : ""
                    })
                    .on(
                  ...
                  $("<div>").append(
                    $.map(keys(item), key =>
                      $("<p>")
                        ...
                        .css(item[key])
                        .addClass(
                          `${state.activeComponent === key ? "element-on-focus" : ""}`
                        )
                        .text(`${item[key].text}`)
                        .on(
                          "click",
    ...
    activeCombination: (store, handlers) => {
      ...
      $(".active-combination").addClass("loaded").empty().append(
        $.map(availableComponent, key => {
          const totalCount = countTotalPermutation(state.app[key]);
          return $("<div>")
            .addClass(
              `active-combination__item ${state.activeComponent === key ? "is-active" : ""}`
            )
            .attr("component-name", key)
            .on("click", handlers.changeActiveComponent.bind(null, store, view))
            .append(
              $("<p>").addClass("active-combination__identifier").text(key)
            )
            .append(
              $("<p>")
                .addClass("active-combination__count")
                .text(
                  `${totalCount} ${totalCount > 1 ? "combinations" : "combination"}`
                )
            );
        })
      );
    ...
    input: (store, handlers) => {
      ...
      const activeProperties = compose(
        keys,
        path([s.activeComponent, "properties"])
      )(s.app);
      ...
      if (!s.activeComponent) {
        ...
        setTimeout(() => {
          $("#inputs").empty();
          $("#inputs").append(
            $.map(activeProperties, prop => {
              const propertyText = path([
                s.activeComponent,
                "properties",
                prop
              ])(s.app);
              ...
              return $("<div>")
                ...
                .append(
                  $("<label>").text(prop.replace(/([A-Z])/g, " $1")) // space after capital case
                )
                .append(
                  $(`${prop === "text" ? "<textarea>" : "<input>"}`)
